...
with open(gff,"r") as infile:
    for line in infile:
        ...
        if feature.type == "gene":
            ...
            genes[feature.parent].add_transcript(feature)

        elif feature.type == "exon":
            for parent in feature.parent:
                for gene in genes:
                    if len(re.findall(gene,parent)) > 0:
                        genes[gene][parent].add_exon(feature)
                        break
                    else:
...
for gene in genes:
    for transcript in gene.transcript:
        if len(transcript.cds) > len(transcript.exons):
            transcript.exons = []
            for segment in range(0,len(transcript.cds)):
                genes[gene][transcript].cds[segment].type = "exon"
                genes[gene][transcript].cds[segment].offset = "."
...
#  this will effectively delete 1-4 amino acids per adjustment
        for j in range(0,len(transcript.exons)):
            try:
                dist=int(transcript.exons[j+1].start) - int(transcript.exons[j].stop)
                while dist < 11: # if use 'while dist < 10:' then too-small introns remain...
                    dist += 3
                    genes[gene][transcript].exons[j+1].start = str(int(transcript.exons[j+1].start)+3)

                    # adjust the corresponding CDS start too
                    try:
                        for segment in range(0,len(transcript.cds)):
                            if transcript.exons[j+1].start == transcript.cds[segment].start:
                                genes[gene][transcript].cds[segment].start = str(int(transcript.cds[segment].start)+3)
                    except:
                        continue
            except:
                continue
...
scafs = set()
for rec in genes:
    if rec.gene not in scafs:
        tblout.write("\t".join(">FEATURE",rec.gene.seqid) + "\n")
        tblout.write("\t".join("1",len(genome[rec.gene.seqid]),"REFERENCE") + "\n")
        tblout.write("".join("\t\t\tPBARC","12345") + "\n")

    tblout.write(rec.print_gene(format="tbl") + "\n")

    for prod in rec.transcript:
        tblout.write(prod.print_transcript(format="tbl") + "\n")
